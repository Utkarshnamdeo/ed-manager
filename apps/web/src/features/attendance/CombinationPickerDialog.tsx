...
import { type Student, type ClassSession, type PricingConfig, type AttendanceCombination, type AttendanceRecord, type CombinationToken, MembershipTier, ClassCardType, ClassType, ExternalProvider } from '../../types';
...
function TokenButton({ label, sublabel, selected, disabled, onToggle }: {
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={`flex flex-col items-center justify-center px-2 py-2.5 rounded-[0.625rem] border text-center transition-[background-color,border-color,opacity] duration-100 cursor-pointer min-h-[56px] ${ selected
          ...
          <div className="px-5 py-4 flex flex-col gap-3 overflow-y-auto">
            ...
            {passType && (
              <div>
                ...
                <div className="grid gap-2" style={{ gridTemplateColumns: passType === MembershipTier.Gold ? '1fr' : '1fr' }}>
                  {passType === MembershipTier.Gold && (
                    <TokenButton
                      label={t('combination.gold')}
                      sublabel={t('combination.unlimited')}
                      selected={isSelected(MembershipTier.Gold)}
                      disabled={isDisabled(MembershipTier.Gold)}
                      onToggle={() => toggleToken(MembershipTier.Gold)}
                    />
                  )}
                  {passType === MembershipTier.Silver && (
                    <TokenButton
                      ...
                      selected={isSelected(MembershipTier.Silver)}
                      disabled={isDisabled(MembershipTier.Silver) || (creditsLeft !== null && creditsLeft < 1)}
                      onToggle={() => toggleToken(MembershipTier.Silver)}
                    />
                  )}
                  {passType === MembershipTier.Bronze && (
                    <TokenButton
                      label={t('combination.bronze')}
                      sublabel={creditsLeft !== null
                        ? `${ creditsNeeded } ${ t('combination.credits') } · ${ t('combination.creditsLeft', { count: creditsLeft }) }`
                        : `${ creditsNeeded } ${ t('combination.credits') }`}
                      selected={isSelected(MembershipTier.Bronze)}
                      disabled={isDisabled(MembershipTier.Bronze) || (creditsLeft !== null && creditsLeft < 1)}
                      onToggle={() => toggleToken(MembershipTier.Bronze)}
                    />
                  )}
                  {(passType === ClassCardType.TenClass || passType === ClassCardType.FiveClass) && (
                    <TokenButton
                      label={passType === ClassCardType.TenClass ? t('combination.tenClassCard') : t('combination.fiveClassCard')}
                      sublabel={creditsLeft !== null
                        ? `${ creditsNeeded } ${ t('combination.credits') } · ${ t('combination.creditsLeft', { count: creditsLeft }) }`
                        : `${ creditsNeeded } ${ t('combination.credits') }`}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <TokenButton
                  ...
                  selected={isSelected(ExternalProvider.USC)}
                  disabled={isDisabled(ExternalProvider.USC)}
                  onToggle={() => toggleToken(ExternalProvider.USC)}
                />
                <TokenButton
                  label={t('combination.eversports')}
                  sublabel={pricingConfig ? `€${ pricingConfig.eversportsRatePerCheckin }` : undefined}
                  selected={isSelected(ExternalProvider.Eversports)}
                  disabled={isDisabled(ExternalProvider.Eversports)}
                  onToggle={() => toggleToken(ExternalProvider.Eversports)}
                />
              </div>
            </div>
